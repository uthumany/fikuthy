...
use fikuthy_core::MessageRole;
use fikuthy_provider::{
    has_provider_configuration, supported_providers, ChatMessage, Gateway, ProviderKind,
};
use fikuthy_security::Policy;
use fikuthy_storage::Storage;
...
#[derive(Parser, Debug)]
#[command(name = "fikuthy", version = VERSION, about = "Fikuthy Agent Terminal — local-first autonomous work")]
struct Cli {
...
struct App {
    storage: Storage,
    workspace: fikuthy_core::Workspace,
}
...
impl App {
    fn open(path: impl AsRef<Path>) -> Result<Self> {
        ...
        let data_dir = env::var_os("FIKUTHY_HOME")
            .map(PathBuf::from)
            .unwrap_or_else(|| {
                dirs_fallback()
                    .join(".local")
        ...
        let db_path = env::var_os("FIKUTHY_DB")
            .map(PathBuf::from)
            .unwrap_or_else(|| data_dir.join("fikuthy.db"));
        let storage = Storage::open(db_path)?;
        let workspace = storage.ensure_workspace(&workspace_path)?;
        Ok(Self { storage, workspace })
    }

    fn current_session(&self) -> Result<fikuthy_core::Session> {
        self.storage
            .list_sessions(self.workspace.id)?
            .into_iter()
            .next()
            .ok_or_else(|| anyhow::anyhow!("no session exists; run `fikuthy sessions new`"))
    }

    fn ensure_session(&self, title: &str) -> Result<fikuthy_core::Session> {
        match self
            .storage
            .list_sessions(self.workspace.id)?
            .into_iter()
...
fn main() -> Result<()> {
    ...
    let cli = Cli::parse();
    if cli.no_banner {
        env::set_var("FIKUTHY_BANNER", "hide");
    }
    ...
    match cli.command {
        ...
        Some(CommandKind::Init(args)) => {
            ...
            println!("FIKUTHY initialized");
            println!("workspace: {}", app.workspace.canonical_path);
            println!("database:  {}", app.storage.path().display());
            println!(
                "next:      fikuthy sessions new && fikuthy chat \"Inspect this workspace\""
            );
            Ok(())
        }
...
fn skills_command(args: SkillsArgs) -> Result<()> {
    ...
    match args.action.unwrap_or(SkillAction::List { limit: 40 }) {
        SkillAction::List { limit } => {
            println!("FIKUTHY SKILL REGISTRY · {} indexed", registry.count()?);
...
fn chat(args: ChatArgs) -> Result<()> {
    ...
    let response = match Gateway::from_environment() {
        Ok(provider) => {
            live = true;
            println!("Fiku · {}/{}", provider.provider(), provider.display_model());
            use std::io::Write;
            io::stdout().flush()?;
            match provider.complete_streaming(
                &[ChatMessage { role: "system".into(), content: "You are Fiku, a concise terminal coding agent. Never claim a command ran unless a tool result proves it.".into() }, ChatMessage { role: "user".into(), content: args.prompt.clone() }],
                |delta| { print!("{delta}"); io::stdout().flush()?; Ok(()) },
            ) {
                Ok(response) => { println!(); response }
                Err(error) => {
                    live = false;
                    eprintln!("warning: provider failed ({}), falling back to offline planner", error);
                    format!("Offline planner ready. I received: {}\n\nConfigure a valid provider with `fikuthy providers env` to enable live model streaming.", args.prompt)
                }
            }
        }
        Err(_error) if !has_provider_configuration() =>
            format!("Offline planner ready. I received: {}\n\nConfigure a provider with `fikuthy providers env` to enable live model streaming.", args.prompt),
...
fn autonomous(args: AutonomousArgs) -> Result<()> {
    ...
    let planner_prompt = format!(
        "You are the Fikuthy autonomous planner. Return only valid JSON with this shape: {{\"summary\":\"short summary\",\"steps\":[{{\"tool\":\"list_directory|read_file|git_status|git_diff\",\"target\":\"relative path or null\",\"rationale\":\"short reason\"}}],\"final_response\":\"short completion note\"}}. Plan at most {max_steps} read-only steps. Never request shell, write, network, secrets, or paths outside the workspace. Candidate skills from the local registry: {recommended_names}. Loaded skill evidence: {}. Task: {}",
        skill_evidence.join("; "),
        args.prompt
    );
    ...
    for (index, step) in plan.steps.into_iter().take(max_steps).enumerate() {
        if !autonomous_tool_enabled(&step.tool) {
            ...
            println!("   disabled by fikuthy.json capability selection");
            continue;
        }
        let request = fikuthy_core::ToolRequest {
        ...
        if decision != fikuthy_core::PermissionDecision::Allow {
...
fn run_command(args: RunArgs) -> Result<()> {
    if !runtime_tool_enabled("terminal") {
        anyhow::bail!("terminal capability is disabled; enable it with `fikuthy setup`");
    }
    ...
    match action {
        SessionAction::List => {
            ...
            if sessions.is_empty() {
                println!("No sessions. Create one with `fikuthy sessions new`. ");
            }
...
fn memory(action: MemoryAction) -> Result<()> {
    ...
    match action {
        MemoryAction::List => {
            println!("MEMORY");
            println!("workspace: {}", app.workspace.canonical_path);
            println!("Use `fikuthy memory search <query>` to search persisted records.");
        }
...
fn doctor(args: DoctorArgs) -> Result<()> {
    if args.fix {
        ...
        println!("FIKUTHY DOCTOR --FIX");
        println!("✓ repaired      user configuration directory");
    }
    let app = App::open(".")?;
    println!("FIKUTHY DOCTOR");
    ...
    match Gateway::from_environment() {
        ...
        Err(_) => println!("! provider      offline planner only; run `fikuthy providers env`"),
    }
...
fn setup(args: SetupArgs) -> Result<()> {
    ...
    if mode == "import" {
        ...
        let destination = env::current_dir()?.join("fikuthy.json");
        fs::write(
            &destination,
    ...
    if let Some(url) = args.provider_url.as_deref() {
        env::set_var("FIKUTHY_PROVIDER_URL", url);
    }
    if let Some(model) = args.model.as_deref() {
        env::set_var("FIKUTHY_MODEL", model);
    }
    ...
    let config_path = env::current_dir()?.join("fikuthy.json");
    fs::write(
        &config_path,
    ...
    println!("\nAPI keys are never stored in fikuthy.json or logs; setup secrets use a private secrets.env file.");
    if let Some(status) = status {
        if status.configured {
            ...
            println!(
                "! credentials missing; run `fikuthy providers env` for {} setup",
                config.provider
            );
        }
    }
    println!("next: fikuthy providers test && fikuthy doctor && fikuthy");
    Ok(())
}
...
fn providers(action: ProviderAction) -> Result<()> {
    match action {
        ...
        ProviderAction::Env => {
            ...
            println!("FIKUTHY_PROVIDER=openrouter|openai|groq|together|deepseek|fireworks|nvidia|ollama|custom");
            println!("FIKUTHY_MODEL=<provider model id>");
            println!("FIKUTHY_PROVIDER_URL=<HTTPS OpenAI-compatible /v1 endpoint>");
            println!("FIKUTHY_API_KEY=<custom override>");
            ...
            println!("Secrets are read at process start and are never persisted by Fikuthy.");
        }
    }
    Ok(())
}
...
fn agents(action: AgentAction) -> Result<()> {
    match action {
        AgentAction::List => {
            ...
            println!("● Fiku       planner/executor   READY");
            ...
            println!("Run: fikuthy agents run \"Inspect this repository\"");
            Ok(())
        }
        AgentAction::Run {
            prompt,
    ...
    match action {
        ModelAction::List => {
            ...
            if let Ok(gateway) = Gateway::from_environment() {
                ...
                println!("active: {}/{}", gateway.provider(), gateway.display_model());
            } else {
                for provider in supported_providers()
                    .into_iter()
...
fn update() -> Result<()> {
    ...
    let release_install = install_directory.join("fikuthy-ui").is_dir()
    ...
    if !release_install {
        ...
        println!("  npm install --global fikuthy@latest");
        println!("  python -m pip install --upgrade fikuthy");
        println!("  uv tool upgrade fikuthy");
        println!("  cargo install --git https://github.com/fikuthy/fikuthy --package fikuthy-cli --locked --force");
        return Ok(());
    }
    ...
    #[cfg(windows)]
    {
        ...
        println!("  irm https://raw.githubusercontent.com/fikuthy/fikuthy/main/packaging/install.ps1 | iex");
    }

    #[cfg(not(windows))]
    {
        const INSTALLER_URL: &str =
            "https://raw.githubusercontent.com/fikuthy/fikuthy/main/packaging/install.sh";
        println!(
        ...
        let temporary = env::temp_dir().join(format!("fikuthy-update-{}.sh", std::process::id()));
        ...
        println!("✓ Fikuthy release installation updated");
    }
    Ok(())
}

fn uninstall() -> Result<()> {
    println!("FIKUTHY UNINSTALL");
    if termux::is_termux() {
        let paths = termux::paths();
        println!("Run: pkg uninstall fikuthy");
        ...
        println!(
            "Use the package manager that installed fikuthy, or remove the source checkout."
        );
    }
    Ok(())
}
...
fn print_termux_doctor() {
    ...
    if warnings == 0 {
        ...
        println!("\n{warnings} warning(s); core FIKUTHY remains usable where optional capabilities are unavailable.");
    }
}
...
fn config_set(key: &str, value: &str) -> Result<()> {
    ...
    let path = env::current_dir()?.join("fikuthy.json");
    fs::write(
        &path,
        format!("{}\n", serde_json::to_string_pretty(&config)?),
    )?;
...
fn launch_tui(headless: bool) -> Result<()> {
    if headless || !io::stdin().is_terminal() || !io::stdout().is_terminal() {
        banner::print_startup_banner(VERSION)?;
        banner::print_onboarding_tips()?;
        println!("FIKUTHY · AGENT TERMINAL");
        println!(
            "● ONLINE · offline planner · workspace {}",
            env::current_dir()?.display()
        );
        println!(
            "Persistent TUI requires an interactive terminal. Try `fikuthy tui` from a terminal."
        );
        return Ok(());
    }
...
fn launch_ui(arguments: &[&str]) -> Result<()> {
    let repository_root = env::var_os("FIKUTHY_SOURCE_ROOT")
        .map(PathBuf::from)
        .or_else(|| {
    ...
    let mut installed_ui_directories = executable_directory
        .clone()
        .map(|path| vec![path.join("ui"), path.join("fikuthy-ui")])
        .unwrap_or_default();
    if let Some(prefix) = env::var_os("PREFIX").map(PathBuf::from) {
        installed_ui_directories.push(prefix.join("lib").join("fikuthy"));
        installed_ui_directories.push(prefix.join("share").join("fikuthy"));
    }
    let ui_directory = installed_ui_directories
        .into_iter()
        .find(|path| path.join("dist/index.js").is_file())
        .unwrap_or(repository_ui_directory);
    let ui_entry = env::var_os("FIKUTHY_UI_ENTRY")
        .map(PathBuf::from)
    ...
    let mut command = if ui_entry.is_file() {
        ...
        anyhow::bail!(
            "TypeScript terminal UI is unavailable. Build it with `pnpm --dir {} install && pnpm --dir {} build` or set FIKUTHY_UI_ENTRY.",
    ...
    if let Ok(runtime_binary) = env::current_exe() {
        command.env("FIKUTHY_RUNTIME_BIN", runtime_binary);
    }
...
fn load_runtime_config() -> Result<Option<RuntimeConfig>> {
    ...
    let config = serde_json::from_str(&raw)
        .with_context(|| format!("invalid Fikuthy configuration in {}", path.display()))?;
    Ok(Some(config))
}
...
fn apply_runtime_config() -> Result<()> {
    ...
    if config.schema_version != 1 {
        anyhow::bail!(
            "unsupported fikuthy.json schema version {}",
            config.schema_version
        );
    }
    if config.provider != "offline" && env::var_os("FIKUTHY_PROVIDER").is_none() {
        env::set_var("FIKUTHY_PROVIDER", &config.provider);
    }
    if config.provider != "offline" && env::var_os("FIKUTHY_MODEL").is_none() {
        env::set_var("FIKUTHY_MODEL", &config.model);
    }
    if env::var_os("FIKUTHY_PERMISSION").is_none() {
        env::set_var("FIKUTHY_PERMISSION", &config.permission_mode);
    }
    if env::var_os("FIKUTHY_TOOLS").is_none() {
        env::set_var("FIKUTHY_TOOLS", config.tools.join(","));
    }
    if env::var_os("FIKUTHY_BANNER").is_none() {
        env::set_var(
            "FIKUTHY_BANNER",
            if config.ui.banner {
    ...
    if env::var_os("FIKUTHY_ICONS").is_none() {
        env::set_var("FIKUTHY_ICONS", &config.ui.icons);
    }
    Ok(())
}
