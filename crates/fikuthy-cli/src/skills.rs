...
impl SkillRegistry {
    ...
    pub fn seed_builtins(&self) -> Result<usize> {
        let categories = [
            ...
            (
                "memory",
                "Memory search",
                "Search persisted FIKUTHY project memory",
                "memory-search",
                vec!["memory_search"],
            ),
            (
            ...
            (
                "agents",
                "Agent handoff",
                "Prepare a scoped handoff between FIKUTHY agents",
                "agent-handoff",
                vec!["planner"],
            ),
            (
    ...
    pub fn install(&self, id: &str, allow_external: bool) -> Result<SkillRecord> {
        ...
        fs::write(
            install_path.join("fikuthy.skill.json"),
    ...
    pub fn rollback(&self, id: &str) -> Result<SkillRecord> {
        ...
        let manifest: SkillManifest =
            serde_json::from_slice(&fs::read(install_path.join("fikuthy.skill.json"))?)?;
    ...
    pub fn run(&self, id: &str, workspace: &Path, allow_external: bool) -> Result<String> {
        ...
        if record.status != "installed" && record.manifest.source.provider != "builtin" {
            anyhow::bail!("skill {id} is not installed; run `fikuthy skills install {id}` first")
        }
        if record.manifest.source.provider != "builtin" {
            ...
            anyhow::bail!(
                "skill {id} has no trusted FIKUTHY execution adapter; it remains metadata-only"
            )
        }
        ...
        match id {
            "builtin.repo-inspector" => {
                ...
                if policy.evaluate(&request) != fikuthy_core::PermissionDecision::Allow {
    ...
    fn sync_voltagent(&self, limit: usize) -> Result<SyncReport> {
        ...
        let text = Client::builder()
            .user_agent("fikuthy-skill-engine/0.1")
            .build()?
...
fn builtin_manifest(
    ...
    SkillManifest {
        ...
        source: SkillSource {
            ...
            url: "https://github.com/fikuthy/fikuthy".into(),
            repository: Some("fikuthy/fikuthy".into()),
            commit: None,
        },
        version: "0.1.0".into(),
        ...
        tags: vec![category.into(), "fikuthy".into(), "safe".into()],
        ...
        compatibility: SkillCompatibility {
            ...
            notes: vec!["Built into the FIKUTHY runtime".into()],
        },
        license: Some("MIT".into()),
        homepage: Some("https://github.com/fikuthy/fikuthy".into()),
        documentation: Some(description.into()),
        checksum: None,
        update_source: Some("fikuthy".into()),
    }
}
