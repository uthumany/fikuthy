...
fn run_with_env(
    ...
    command
        .current_dir(cwd)
        .env("HOME", home)
        .env("FIKUTHY_HOME", home.join(".fikuthy"))
        .args(args);
    for (key, value) in extra_env {
        command.env(key, value);
    }
    let output = command.output().expect("run fikuthy");
    assert!(
...
fn cli_version_matches_cargo_package_version() {
    let output = Command::new(env!("CARGO_BIN_EXE_fikuthy"))
        .arg("--version")
        .output()
        .expect("run fikuthy --version");
    assert!(output.status.success());
    let reported = String::from_utf8(output.stdout).expect("utf8 version output");
    assert_eq!(
        reported.trim(),
        format!("fikuthy {}", env!("CARGO_PKG_VERSION"))
    );
}
...
fn skill_commands_cover_registry_lifecycle_and_local_import() {
    ...
    let bin = env!("CARGO_BIN_EXE_fikuthy");
    ...
    assert!(list.contains("FIKUTHY SKILL REGISTRY"));
    ...
    let manifest = workspace.path().join("fikuthy.skill.json");
    std::fs::write(
        &manifest,
...
fn termux_commands_create_no_root_paths_and_report_optional_features() {
    ...
    assert!(home.path().join(".config/fikuthy").is_dir());
    assert!(home.path().join(".local/share/fikuthy/skills").is_dir());
    assert!(home.path().join(".cache/fikuthy").is_dir());
    ...
    assert!(doctor_command.contains("FIKUTHY TERMUX DOCTOR"));
    ...
    assert!(doctor.contains("FIKUTHY TERMUX DOCTOR"));
...
fn cli_persists_workspace_session_memory_and_doctor() {
    ...
    assert!(init.contains("FIKUTHY initialized"));
...
fn setup_writes_valid_runtime_configuration_without_secrets() {
    ...
    let raw = std::fs::read_to_string(workspace.path().join("fikuthy.json")).unwrap();
    ...
    let global = std::fs::read_to_string(home.path().join(".fikuthy/config.yaml")).unwrap();
    ...
    let updated = std::fs::read_to_string(workspace.path().join("fikuthy.json")).unwrap();
...
fn setup_scans_and_persists_validated_secrets_outside_configuration() {
    ...
    let mut child = Command::new(bin)
        .current_dir(workspace.path())
        .env("HOME", home.path())
        .env("FIKUTHY_HOME", home.path().join(".fikuthy"))
        .env("FIKUTHY_PROVIDER_URL", format!("http://{address}/v1"))
        .args([
            "setup",
            "--non-interactive",
            "--provider",
    ...
    let config = std::fs::read_to_string(workspace.path().join("fikuthy.json")).unwrap();
    assert!(!config.contains("setup-secret"));
    let secrets = home.path().join(".fikuthy/secrets.env");
...
fn chat_streams_from_an_openai_compatible_gateway_and_persists_the_result() {
    ...
    let output = run_with_env(
        env!("CARGO_BIN_EXE_fikuthy"),
        workspace.path(),
        home.path(),
        &["chat", "hello gateway"],
        &[
            ("FIKUTHY_PROVIDER", "custom"),
            ("FIKUTHY_PROVIDER_URL", &endpoint),
            ("FIKUTHY_MODEL", "fixture-model"),
            ("FIKUTHY_API_KEY", "test-secret"),
        ],
    );
    server.join().unwrap();
    assert!(output.contains("Fiku · custom/fixture-model"));
