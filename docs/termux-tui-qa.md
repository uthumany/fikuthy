...
The PTY harness was run with `TERMUX_VERSION=0.118.0` and a Termux-style `PREFIX` across 40, 60, 80, 120, 160, and 220 columns. The compact 40-column capture preserved the FIKUTHY branding, reduced the header to a short command hint, used the compact `Ask FIKUTHY…` composer, and kept status content within the terminal frame. The 60-column capture used the Termux header, displayed the `↑↓ navigate · Enter select` hint, kept the full message composer within the frame, and rendered the status bar with truncation rather than horizontal overflow.
