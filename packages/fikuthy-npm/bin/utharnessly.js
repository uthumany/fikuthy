...
const REPOSITORY = 'fikuthy/fikuthy';
const BASE_URL = (process.env.FIKUTHY_RELEASE_BASE_URL || `https://github.com/${REPOSITORY}/releases/download/v${VERSION}`).replace(/\/$/, '');

function platformAsset() {
  const platform = process.platform;
  const arch = process.arch;
  if (platform === 'linux' && arch === 'x64') return ['fikuthy-linux-x64.tar.gz', 'tar.gz'];
  if (platform === 'darwin' && arch === 'x64') return ['fikuthy-macos-x64.tar.gz', 'tar.gz'];
  if (platform === 'darwin' && arch === 'arm64') return ['fikuthy-macos-arm64.tar.gz', 'tar.gz'];
  if (platform === 'win32' && arch === 'x64') return ['fikuthy-windows-x64.zip', 'zip'];
  throw new Error(`No published fikuthy binary for ${platform}/${arch}. Supported release targets are Linux x64, macOS x64/arm64, and Windows x64; use the source instructions at https://github.com/${REPOSITORY} on other targets.`);
}
...
function cacheRoot() {
  ...
  return path.join(base, 'fikuthy', VERSION);
}
...
async function ensureBinary(force = false) {
  ...
  const binary = path.join(root, process.platform === 'win32' ? 'fikuthy.exe' : 'fikuthy');
  if (!force) {
    try { await fs.access(binary); return { binary, ui: path.join(root, 'ui') }; } catch {}
  }
  await fs.rm(root, { recursive: true, force: true });
  await fs.mkdir(root, { recursive: true });
  const temp = await fs.mkdtemp(path.join(os.tmpdir(), 'fikuthy-'));
  const archive = path.join(temp, asset);
  const checksums = path.join(temp, 'SHA256SUMS');
  try {
    process.stderr.write(`Downloading fikuthy v${VERSION} (${process.platform}/${process.arch})…\n`);
    ...
    const packageRoot = (await fs.readdir(extracted, { withFileTypes: true })).find((entry) => entry.isDirectory() && entry.name.startsWith('fikuthy-'));
    if (!packageRoot) throw new Error('release archive did not contain an fikuthy directory');
...
function run(binary, args) {
  const child = spawn(binary, args, { stdio: 'inherit', env: { ...process.env, FIKUTHY_RUNTIME_BIN: binary } });
  child.on('error', (error) => { console.error(`fikuthy: ${error.message}`); process.exitCode = 1; });
  child.on('exit', (code, signal) => { process.exitCode = signal ? 1 : (code ?? 1); });
}

const args = process.argv.slice(2);
if (args.includes('--version') || args.includes('-V')) {
  console.log(`fikuthy ${VERSION}`);
  process.exit(0);
}
if (args[0] === 'update') {
  try { await ensureBinary(true); console.log(`fikuthy ${VERSION} is ready.`); } catch (error) { console.error(`fikuthy update failed: ${error.message}`); process.exitCode = 1; }
} else if (args[0] === 'uninstall') {
  console.log('Remove the npm package with: npm uninstall -g fikuthy');
  ...
  try {
    ...
    console.error(`fikuthy: ${error.message}`);
    process.exitCode = 1;
  }
}
