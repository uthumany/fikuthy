"""Thin Python launcher for the fikuthy native release."""
...
def platform_asset() -> tuple[str, str]:
    ...
    if system.startswith("linux") and machine in {"x86_64", "amd64"}:
        return "fikuthy-linux-x64.tar.gz", "tar.gz"
    if system == "darwin" and machine in {"x86_64", "amd64"}:
        return "fikuthy-macos-x64.tar.gz", "tar.gz"
    if system == "darwin" and machine in {"arm64", "aarch64"}:
        return "fikuthy-macos-arm64.tar.gz", "tar.gz"
    if system.startswith("win") and machine in {"x86_64", "amd64"}:
        return "fikuthy-windows-x64.zip", "zip"
    raise RuntimeError(
        f"No published fikuthy binary for {system}/{machine}. "
...
def cache_root() -> Path:
    ...
    return base / "fikuthy" / VERSION


def _download(url: str, destination: Path) -> None:
    request = urllib.request.Request(url, headers={"User-Agent": "fikuthy-python-launcher"})
...
def ensure_binary(force: bool = False) -> tuple[Path, Path]:
    ...
    root = cache_root()
    binary = root / ("fikuthy.exe" if os.name == "nt" else "fikuthy")
    ui = root / "ui"
    if not force and binary.is_file():
        return binary, ui
    shutil.rmtree(root, ignore_errors=True)
    root.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="fikuthy-") as temporary:
        temporary_path = Path(temporary)
        archive = temporary_path / asset
        checksums = temporary_path / "SHA256SUMS"
        print(f"Downloading fikuthy v{VERSION} ({sys.platform}/{platform.machine()})…", file=sys.stderr)
        ...
        package_root = next((entry for entry in extracted.iterdir() if entry.is_dir() and entry.name.startswith("fikuthy-")), None)
        if package_root is None:
            raise RuntimeError("release archive did not contain an fikuthy directory")
...
def main() -> int:
    ...
    if args in (["--version"], ["-V"]):
        print(f"fikuthy {VERSION}")
        return 0
    if args and args[0] == "update":
        try:
            ensure_binary(force=True)
            print(f"fikuthy {VERSION} is ready.")
            return 0
        except Exception as error:  # pragma: no cover - platform/network-specific
            print(f"fikuthy update failed: {error}", file=sys.stderr)
            return 1
    if args and args[0] == "uninstall":
        print("Remove the Python package with: python -m pip uninstall fikuthy")
    ...
    try:
        ...
        print(f"fikuthy: {error}", file=sys.stderr)
        return 1
    environment = os.environ.copy()
    environment["FIKUTHY_RUNTIME_BIN"] = str(binary)
