...
# Signing is mandatory for publishable output. Set FIKUTHY_GPG_KEY_ID and
...
KEY_ID="${FIKUTHY_GPG_KEY_ID:-}"
VERSION="${FIKUTHY_VERSION:-0.2.13}"

if [[ -z "$KEY_ID" && "${FIKUTHY_ALLOW_UNSIGNED:-0}" != "1" ]]; then
  echo 'FIKUTHY_GPG_KEY_ID is required for a publishable signed repository.' >&2
  echo 'Set FIKUTHY_ALLOW_UNSIGNED=1 only for local metadata tests.' >&2
  exit 2
fi
...
rm -rf "$OUTPUT_DIR"
mkdir -p "$OUTPUT_DIR/dists/stable/main" "$OUTPUT_DIR/pool/main" "$OUTPUT_DIR/pool/main/fikuthy"
shopt -s nullglob
packages=("$INPUT_DIR"/*.deb)
((${#packages[@]} > 0)) || { echo "No .deb packages found in $INPUT_DIR" >&2; exit 3; }
for package in "${packages[@]}"; do
  cp "$package" "$OUTPUT_DIR/pool/main/fikuthy/"
done
...
for arch in aarch64 x86_64; do
  ...
  : > "$packages_file"
  for package in "$OUTPUT_DIR/pool/main/fikuthy/"*_"$arch".deb; do
    [[ -f "$package" ]] || continue
...
cat > "$release" <<RELEASE
Origin: FIKUTHY
Label: FIKUTHY Termux
Suite: stable
Codename: stable
Version: $VERSION
Architectures: aarch64 x86_64
Components: main
Description: Signed FIKUTHY packages for Termux
Date: $(date -Ru)
MD5Sum:
RELEASE
...
if [[ -n "$KEY_ID" ]]; then
  ...
  gpg --batch --yes --export "$KEY_ID" > "$OUTPUT_DIR/fikuthy.gpg"
...
tar -C "$(dirname "$OUTPUT_DIR")" -czf "$(dirname "$OUTPUT_DIR")/fikuthy-termux-repository-${VERSION}.tar.gz" "$(basename "$OUTPUT_DIR")"
sha256sum "$(dirname "$OUTPUT_DIR")/fikuthy-termux-repository-${VERSION}.tar.gz" > "$(dirname "$OUTPUT_DIR")/fikuthy-termux-repository-${VERSION}.tar.gz.sha256"
printf '%s\n' "$OUTPUT_DIR"
